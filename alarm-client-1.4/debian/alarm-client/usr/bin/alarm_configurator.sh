...
while [[ "$_choose_location" == "" ]]; do
	...
	case $? in
		...
		5) 	rm /tmp/alarm_session.conf
			alarm_configurator.sh &
			exit 0;;
#was:		*) _configfile="/etc/alarm/alarm_local.conf";;
		*) _configfile="$HOME/.config/alarm/alarm_user.conf";;
	esac
...
grep -v "$_location" "$_configfile" > /tmp/tmp$(whoami).conf
case ${entry[1]} in
...
echo -e "ALARM['ACTIVE','$_location']=\"${entry[0]}\"\nALARM['AUDIO','$_location']=\"${entry[2]}\"\nALARM['AUDIO_VOLUME','$_location']=\"${entry[3]}\"\nALARM['VIDEO','$_location']=\"${entry[1]}\"" >> /tmp/tmp$(whoami).conf
$_tmp && echo -e "ALARM['TMP','$_location']=\"TRUE\"" >> /tmp/tmp$(whoami).conf
mv /tmp/tmp$(whoami).conf "$_configfile"
