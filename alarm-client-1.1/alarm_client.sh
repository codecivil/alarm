		...
		"CREATE")
			...
			if [[ ${ALARM['VIDEO',${ALARM['LOCATION']}]} != "" ]]; then
			{
				...
				if [[ "$(grep -i message ${ALARM['LOCATION_FILE']})" != "" ]]; then
					...
					_bg_color="#ff0000" #red
					_fg_color="#ffffff" #white
				fi
				...
				_yad "create.${ALARM['DESCRIPTION',${ALARM['LOCATION']}]}" --title "" --text-info --filename="alarm.txt" --text="<span foreground=\"red\" size=\"x-large\">$_morealarms</span>" --back="$_bg_color" --fore="$_fg_color"  --justify='center' --no-buttons --width=500 --height=300 --sticky --on-top --fontname="Sans Bold $_size" $_fullscreen &
			}& 
			fi
		...
		"DELETE")
			...
			if [[ "${ALARM['VIDEO',${ALARM['LOCATION']}]}" != "" ]]; then
			{
				if [[ "${ALARM['IP',${ALARM['LOCATION']}]}" == *"${ALARM['MYIP']}"*  || "${ALARM['NOALARM',${ALARM['LOCATION']}]}" == *"${ALARM['MYIP']}"* ]]; then
					...
					_yad "delete.${ALARM['DESCRIPTION',${ALARM['LOCATION']}]}" --title "" --text-info --text="<span foreground=\"red\" size=\"x-large\">$_morealarms</span>" --filename="alarm.txt" --back='#00ff00' --fore='#464646'  --justify='center' --no-buttons --width=500 --height=300 --sticky --on-top --fontname="Sans Bold $_size" $_fullscreen &
				fi
			}& 
			fi
			;;
